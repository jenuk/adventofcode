...
def load_json(leaderboard_id: str) -> dict:
    with open(f"{leaderboard_id}.json") as file:
...
def json_to_dataframe(data_json: dict, year: int) -> pd.DataFrame:
    ...
    for key in other_keys:
        data[key] = []
    ignore_empty_members = st.checkbox("Ignore members without participation",
                                       value=True)
    for _, member in data_json["members"].items():
        ...
        for k in range(1,26):
            ...
            for p in range(1,3):
                if (str(k) in completions
                        and str(p) in completions[str(k)]):
...
def main():
    # load data
    cols = st.columns(2)
    leaderboard_id = cols[0].text_input("leaderboard ID", 1606205)
    year = cols[1].number_input("Current year", 2015, 2022, 2022)
    data_json = load_json(leaderboard_id)
    ...
    df = df.sort_values(by=["stars", "total_dt_score", "total_local_score"],
                        ascending=False)

    # display table
    print_keys = st.multiselect("Which keys should be shown?",
                                df.columns,
                                ["stars",
                                 "local_score",
                                 "global_score",
